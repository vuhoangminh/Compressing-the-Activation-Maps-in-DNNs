...
from WaveletCompressedConvolution.WCC.WCC import WCC
from WaveletCompressedConvolution.WCC.QuantConv2d import QuantConv2d
    ...
    if isinstance(module, nn.Conv2d):
        ...
        for name, child in module.named_children():
            new_module.add_module(
                name, wavelet_module(child, levels, compress_rate, bit_w, bit_a)
            )
    return new_module
...
def wavelet_resnet(model, levels, compress_rate, bit_w, bit_a):
    wavelet_module(model, levels, compress_rate, bit_w, bit_a)


def main():
    model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes=100)
    wavelet_resnet(model, levels=3, compress_rate=0.5, bit_w=4, bit_a=4)
